...
//     id: circuit.id,
//     title: circuit.titre,
//     location: `${circuit.destination.nom},
...
const PopularCircuits = () => {
  ...
  const circuits =
    data?.allCircuits?.length > 0
      ? data.allCircuits.slice(-4).reverse().map(convertCircuitData) // Remove the duplicate convertCircuitData call
      : [];
