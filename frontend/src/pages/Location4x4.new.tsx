...
const Location4x4: React.FC = () => {
  ...
  const filteredVehicles = vehicles.filter((vehicle) => {
    if (filters.type && vehicle.type.libelle !== filters.type) return false;
    if (filters.capacity && vehicle.capacite.nombre_places < filters.capacity) return false;
    if (filters.priceRange) {
      const [min, max] = filters.priceRange;
      if (vehicle.prix_par_jour < min || vehicle.prix_par_jour > max) return false;
    }
    if (filters.transmission && vehicle.transmission !== filters.transmission) return false;
    if (filters.fuelType && vehicle.carburant !== filters.fuelType) return false;
    return true;
  });
    ...
    <div className="container mx-auto py-8">
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <select
          ...
          onChange={e => setFilters(f => ({ ...f, capacity: e.target.value ? Number(e.target.value) : null }))}
        >
        ...
        <select
          ...
          onChange={e => {
            ...
            setFilters(f => ({
              ...f,
              priceRange: val ? val.split('-').map(Number) as [number, number] : null
            }));
          }}
        >
        ...
        <select
          ...
          onChange={e => setFilters(f => ({ ...f, transmission: e.target.value ? e.target.value as 'MANUELLE' | 'AUTOMATIQUE' : null }))}
        >
        ...
        <select
          ...
          onChange={e => setFilters(f => ({ ...f, fuelType: e.target.value ? e.target.value as 'DIESEL' | 'ESSENCE' : null }))}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {filteredVehicles.length === 0 ? (
          <div className="col-span-full text-center text-gray-500">Aucun véhicule ne correspond à vos critères.</div>
        ) : (
