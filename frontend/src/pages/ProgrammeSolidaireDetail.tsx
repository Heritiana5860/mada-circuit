...
const ProgrammeSolidaireDetail = () => {
  ...
  const nextImage = () => {
    setSelectedImageIndex((prev) =>
      prev === allPangalaneImages.length - 1 ? 0 : prev + 1
    );
  };

  const prevImage = () => {
    setSelectedImageIndex((prev) =>
      prev === 0 ? allPangalaneImages.length - 1 : prev - 1
    );
  };
  ...
  const handleImageError = (index) => {
    console.warn(
      `Erreur de chargement de l'image ${index}: ${allPangalaneImages[index]}`
    );
    if (index === selectedImageIndex && allPangalaneImages.length > 1) {
      setSelectedImageIndex((prev) => (prev + 1) % allPangalaneImages.length);
    }
  };
      ...
      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {/* Contenu principal */}
            <ContenuPrincipal dataFromState={dataFromState} />
...
export default ProgrammeSolidaireDetail;
